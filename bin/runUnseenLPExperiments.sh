#!/bin/bash
#SBATCH -J LinkPredictionUnseenGraphs
#SBATCH -p high
...
for i in `seq 0 $((${#KEYS[@]} - 1))`; do
  ...
  INPUT_PATH="$GRAPH_PATH/$MODEL_GRAPH-C.edgelist"
  ENCODING_PATH="labels/$MODEL_GRAPH-K$_K.json"
  EMB_PATH="emb/$TRAIN_GRAPH-K$_K-D$_D-E$_E-C$_C-M$_M.emb"
  CMD="$LINK_COMMAND '$EMB_PATH' '$INPUT_PATH' '$ENCODING_PATH'";
  OUTPUT="$OUTPUT_DIR/$TRAIN_GRAPH-$MODEL_GRAPH-K$_K-D$_D-E$_E-C$_C-M$_M.unseen.log"
  COMMANDS_ARRAY+=("$CMD")
