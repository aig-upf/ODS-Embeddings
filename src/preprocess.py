...
def compute_degree_counts(G, d, include_center, num_processes):
    params = [(G, n, d, include_center) for n in G]

    if num_processes <= 1:
        all_cds = _batch_compute_node_degree_counts(params)
    else:
        p = Pool(processes=num_processes)
        chunks = [params[x::num_processes] for x in range(num_processes)]
        chunk_cds = p.map(_batch_compute_node_degree_counts, chunks)
